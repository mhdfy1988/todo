...
当前公开版本为 `0.1.0`，属于可试用的早期版本。源码候选版本 `0.1.1` 已增加“复制本周完成”，但尚未发布；Windows x64 NSIS 安装器、公开更新签名与 `latest.json` 已完成首发复验，真正的版本间自动更新仍需用该候选版本完成实机验证。宠物、番茄钟、AI 和日历均不在本阶段范围内。
...
- 当前安装器只面向 Windows x64；`v0.1.0` 的公开下载、元数据、更新签名和当前用户安装基线已经验证，`v0.1.0 → v0.1.1` 应用内更新闭环仍待实机验证。
