...
当前公开版本为 [`0.1.4`](https://github.com/mhdfy1988/todo/releases/tag/v0.1.4)，属于可试用的早期版本。它更正用户可见名称，简化“已完成”页面与入口，稳定父子任务行布局，并换用“悬浮任务牌”软件图标；数据库继续使用 schema v5，内部 `zuoban` 身份、数据目录、更新端点和签名公钥保持不变。Windows x64 NSIS 安装器、更新签名与 `latest.json` 已由发布工作流和匿名公开下载复核。当前正式安装基线为改名前的“代办”`0.1.3`，下一步实测 `0.1.3 → 0.1.4` 应用内更新。宠物、番茄钟、AI 和日历均不在本阶段范围内。

下方功能说明以当前公开版本 `0.1.4` 为准；后续变化继续记录在 [`CHANGELOG.md`](./CHANGELOG.md) 的 `Unreleased`。
...
- 通过拖动柄或 `Alt+↑/↓` 调整完整待办顺序。
- 在待办页和已完成页按 `Ctrl+F`，或从更多菜单进入当前页面搜索。搜索只按标题实时筛选并高亮；搜索期间仍可完成、修改、删除或撤销，但禁止重排过滤后的局部列表。
- 独立查看已完成事项和完成时间，并通过 `↶` 撤销完成；撤销后的任务回到待办队尾。
- 从已完成页右上角更多菜单选择“复制本周完成”，可把本地自然周内仍然有效的完成项复制为 Markdown 编号列表；本周为空时不会覆盖剪贴板。完整周报、上周或自定义日期仍属于后续能力。
- 长列表只在列表内部滚动，录入区和底部入口保持固定；进入已完成页时，返回入口与页名直接替换顶部应用标题，不再额外占用一行。
- 正常模式启动后会检查更新，并每 24 小时再次检查；也可从更多菜单手动检查。发现新版本后，由用户明确点击安装。

### 0.1.4：界面与品牌收口

- 用户可见名称由“代办”更正为“待办”，但内部应用标识、数据目录和更新信任身份保持不变。
- “完成记录”简化为“已完成”：返回入口与页名合并进顶部窗口栏，“复制本周完成”收进该页更多菜单；待办页底部只让右侧“已完成 + 数量 + 箭头”紧凑按钮响应点击，减少误触。
- 父待办与子待办的可选操作统一靠右；删除入口隐藏后，排序柄不再移位。展开子列表时父行保持原位置，编辑态左侧橙色标记随首行居中。
- 软件图标改为深色底座与珊瑚橙任务牌组成的“悬浮任务牌”，桌面、托盘和安装器复用同一源文件生成的图标资源。
...
- 搜索会保留父项上下文和整组真实进度；已完成事项按父项分组并默认折叠；胶囊显示“父项 / 当前子代办”以及父项期限。父项在本周最终完成时，周回顾会把更早完成且仍有效的子项作为缩进明细；普通子项只有在本周完成时才单独进入本周结果。
...
产物位于 `src-tauri/target/release/zuoban-desktop-spike.exe`。用户可见产品名与 Windows 文件元数据为“待办”；内部 identifier、EXE 名和数据库文件名继续保留 `zuoban` 兼容名称，避免破坏已有本地数据。
...
安装器与 `.sig` 更新签名位于 `src-tauri/target/release/bundle/nsis/`。[`v0.1.4` GitHub Release](https://github.com/mhdfy1988/todo/releases/tag/v0.1.4) 的线上 `latest.json`、更新签名和安装器 SHA-256 已由发布工作流与匿名公开下载双重验证；完整版本间升级闭环仍待当前已安装的 `0.1.3` 实测。
...
候选配置使用独立 identifier 和应用数据目录，不覆盖正式“待办”；同时关闭 updater 产物和 Authenticode 签名，只用于本机功能试用，不能作为正式 Release 资产。
...
- 当前安装器只面向 Windows x64；`v0.1.4` 的公开下载、元数据和更新签名已完成验证，`v0.1.3 → v0.1.4` 应用内发现、安装重启与数据保留仍待实机验证。
