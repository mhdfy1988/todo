...
- 每个父待办行右侧固定一个低存在感 `＋`，作为唯一的子代办新增入口；有子代办后，标题后的紧凑进度只负责展开或收起。搜索时隐藏新增入口，删除和排序按钮在新增、编辑过程中保持原位置。
- 点击 `＋` 后，输入框位于子列表末尾。非空标题按 `Enter` 或移开焦点创建；空白失焦或按 `Esc` 直接取消。没有旧子项时取消会恢复收起，已有子项时仍保持展开，成功创建第一条后也保持展开。
- 子标题支持原位修改，按 `Enter` 或失焦保存；子项还可软删除、完成、撤销完成，并在同一父项内拖动或使用 `Alt+↑/↓` 重排。
...
- `0.1.3` 的子代办只支持一级，不提供子项期限、子项金币、跨父项移动、父子转换、批量粘贴或 AI 自动拆解。
...
- [一级子代办早期交互原型（`v0.1.2` 前历史草案）](./docs/prototypes/subtasks-v0.1/index.html)
