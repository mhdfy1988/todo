...
test("正式前端保持一级子代办、独立排序域、后端完成决策和单滚动区契约", async () => {
  ...
  assert.match(views, /item\.append\(checkbox, main, addSubtask, remove, handle, subtaskList\)/);
  ...
  assert.match(styles, /\.task-row\s*\{[^}]*grid-template-columns:\s*20px minmax\(0, 1fr\) 26px 26px 28px;/s);
...
function createCaptureHarness({
  ...
  parentRow.append(addTrigger, subtaskList);
