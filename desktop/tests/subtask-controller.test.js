...
test("零子项的新增输入空白失焦时退出添加态并收起子代办区域", async () => {
  const harness = createCaptureHarness({ subtaskCount: 0 });
  ...
  assert.equal(harness.addTrigger.hidden, false);
  assert.equal(harness.addTrigger.disabled, false);
  assert.equal(harness.subtaskList.hidden, true);
  assert.equal(harness.parentRow.classList.contains("has-expanded-subtasks"), false);
});

test("零子项的新增输入按 Escape 时退出添加态并收起子代办区域", () => {
  const harness = createCaptureHarness({ subtaskCount: 0 });
  harness.controller.startAdding("parent-id");
  const input = harness.captureInput;
  const event = eventFor(input, { key: "Escape" });
...
test("已有子项的新增输入按 Escape 时只取消输入并保持子代办展开", () => {
  const harness = createCaptureHarness();
  harness.controller.startAdding("parent-id");
  const input = harness.captureInput;

  harness.emit("keydown", eventFor(input, { key: "Escape" }));

  assert.equal(harness.captureInput, null);
  assert.equal(harness.addTrigger.hidden, false);
  assert.equal(harness.addTrigger.disabled, false);
  assert.equal(harness.subtaskList.hidden, false);
  assert.equal(harness.parentRow.classList.contains("has-expanded-subtasks"), true);
});
...
test("新增输入保存失败时保留草稿与添加态", async () => {
  ...
  assert.equal(harness.addTrigger.hidden, false);
  assert.equal(harness.addTrigger.disabled, true);
...
test("正式前端保持一级子代办、独立排序域、后端完成决策和单滚动区契约", async () => {
  const [html, styles, views, app, controller, taskListController] = await Promise.all([
    ...
    readFile(new URL("../app/task-list-controller.js", import.meta.url), "utf8"),
  ]);
  ...
  assert.match(views, /addSubtask\.textContent = "＋"/);
  assert.match(views, /addSubtask\.hidden = searching/);
  assert.match(views, /taskActions\.className = "task-actions"/);
  assert.match(views, /taskActions\.append\(addSubtask, remove, handle\)/);
  assert.match(views, /item\.append\(checkbox, main, taskActions, subtaskList\)/);
  assert.match(views, /subtaskActions\.className = "subtask-actions"/);
  assert.match(views, /subtaskActions\.append\(remove, handle\)/);
  assert.match(views, /item\.append\(checkbox, title, subtaskActions\)/);
  assert.match(styles, /\.task-actions,\s*\.subtask-actions \{[^}]*justify-content: flex-end;/);
  assert.doesNotMatch(styles, /grid-template-columns: 20px minmax\(0, 1fr\) 26px 28px/);
  assert.doesNotMatch(views, /subtask-add-row|subtask-add-button|＋ 添加子代办/);
  assert.doesNotMatch(controller, /type = "date"|deadlineOn/);
  assert.doesNotMatch(controller, /subtask-add-row|subtask-add-button|task-edit-add-subtask/);
  assert.doesNotMatch(taskListController, /task-edit-add-subtask|＋ 添加子代办/);
  assert.match(controller, /addTrigger\.hidden = this\.isSearchActive\(\)/);
  assert.match(controller, /addTrigger\.disabled = this\.addingParentId === parentTaskId \|\| !this\.canMutate\(\)/);
  ...
  assert.match(styles, /\.task-row\s*\{[^}]*grid-template-columns:\s*20px minmax\(0, 1fr\) auto;/s);
  assert.match(
    styles,
    /\.task-row\s*\{[^}]*--task-head-height:\s*43px;[^}]*grid-template-rows:\s*minmax\(var\(--task-head-height\), auto\);[^}]*column-gap:\s*9px;[^}]*row-gap:\s*0;/s,
  );
  assert.match(styles, /\.task-row\.is-editing\s*\{[^}]*--task-head-height:\s*77px;[^}]*min-height:\s*78px;/s);
  assert.match(
    styles,
    /\.task-row\[aria-current="step"\]::before\s*\{[^}]*top:\s*calc\(\(var\(--task-head-height\) - 22px\) \/ 2\);[^}]*height:\s*22px;/s,
  );
  assert.match(styles, /\.subtask-row\s*\{[^}]*grid-template-columns:\s*20px minmax\(0, 1fr\) auto;/s);
  assert.match(styles, /\.subtask-add-trigger\s*\{[^}]*width:\s*26px;[^}]*height:\s*28px;/s);
  assert.doesNotMatch(styles, /\.subtask-add-row|\.subtask-add-button|\.task-edit-add-subtask|\.task-row\.is-editing \.subtask-add-trigger/);
...
function createCaptureHarness({
  ...
  parentRow.dataset.subtaskCount = String(subtaskCount);
  ...
  const taskActions = new FakeElement("div", document);
  taskActions.className = "task-actions";
  taskActions.append(addTrigger);
  if (subtaskCount > 0) subtaskList.append(subtaskRow);
  parentRow.append(taskActions, subtaskList);
