...
> 状态：`v0.1.3` 已按标准 CHANGELOG 和签名闭环正式发布为公开 Latest；当前 Windows 正式安装基线仍为 `v0.1.2`，下一步实测应用内更新
> 更新时间：2026-07-24
> 适用版本：`0.1.2 → 0.1.3`
...
| `0.1.2 → 0.1.3` 应用内升级 | 待执行 | 由当前已安装的 `0.1.2` 发现更新，并由用户点击安装后核对重启、版本和数据保留 |
...
当前准确口径是“`v0.1.3` 已正式发布并成为公开 Latest，安装器、更新签名、`latest.json`、Release 正文和公开下载已闭环验证；当前 Windows 正式安装基线仍为 `v0.1.2`，下一步从该真实基线验证应用内发现、安装重启与数据保留”。
