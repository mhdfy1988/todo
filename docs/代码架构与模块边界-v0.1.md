...
> 更新时间：2026-07-27
> 适用范围：Tauri 桌面壳、前端账本会话、本地任务账本、发布更新与联合冒烟；一级子代办与 schema v5 自 `v0.1.2` 起已进入公开版本，`v0.1.5` 发布候选只调整折叠和子列表视觉，不改变架构边界
...
- Windows x64 NSIS 配置、发布版本一致性检查和 GitHub Actions 发布工作流已经落地；截至本轮候选准备，`v0.1.4` 是已核对的公开 Latest，公开 `setup.exe`、`.sig`、`latest.json`、密码学签名与资产摘要均已复验。
- 一级子代办及其 schema v5、完成历史和周回顾语义自 `v0.1.2` 起已经进入公开版本；`v0.1.5` 候选只统一两页折叠入口、透明子列表、组内短轨道和行级反馈，不新增领域状态、IPC 命令或持久化格式。
