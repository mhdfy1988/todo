...
> 状态：当前模块化、统一待办列表、可追溯标题与期限修改、历史保留型删除与窗口反馈收口的实现基线  
> 更新时间：2026-07-18  
> 适用范围：Tauri 桌面壳、前端账本会话、本地任务账本与联合冒烟；不代表阶段 1 全部产品能力已经完成
...
这份文档是代码结构的入口说明；业务状态语义仍以[《任务状态与事件账本 v0.1》](./任务状态与事件账本-v0.1.md)为准，持久化实验结论仍以[《本地事件历史、金币账本与异常恢复技术实验 v0.1》](./本地事件历史、金币账本与异常恢复技术实验-v0.1.md)为准。
...
- 当前统一门禁通过 102 个前端测试与 48 个 Rust 测试；独立账本冒烟和桌面联合冒烟均为 `passed=true`，后者覆盖托盘恢复展开态、未完成标题与期限修改、`updatedPendingTaskDeadline=true`、删除后事件数 9 与完整性。前端测试另覆盖输入法搜索、真实当前项标记、安全高亮、空结果、焦点清理和搜索态重排锁。
