...
假设顶层待办 A 仍在队列中，用户展开 A，并从底部入口依次添加子项 A1、A2：

1. `SubtaskController` 只维护 A 的展开、单次输入和焦点；有效标题按 `Enter` 或失焦通过 `onCreate(parentTaskId, title)` 交给 `LedgerSession.createSubtask`，创建成功后退出输入态并恢复底部入口，不会先把临时子项写入前端快照。
