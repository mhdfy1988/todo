...
> 状态：当前模块化、统一待办列表、可追溯修改、复制本周完成、Windows 打包与自动更新边界的实现基线
> 更新时间：2026-07-22
...
- `WeeklyCompletionController` 复用 `weekly_facts` 查询 `[本周一 00:00, 下周一 00:00)` 的有效完成事件，并生成固定 Markdown；空结果不会写剪贴板。Tauri 官方剪贴板插件只在组合根注册，前端通过写入端口使用最小文本权限。
...
- 源码候选版本为 `v0.1.1`，尚未发布，也尚未完成 `v0.1.0 → v0.1.1` 的应用内升级验证。
