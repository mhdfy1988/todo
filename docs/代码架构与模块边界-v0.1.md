...
> 状态：当前模块化、统一待办列表、一级子代办、可追溯修改、复制本周完成、Windows 打包与自动更新边界的实现基线
> 更新时间：2026-07-27
> 适用范围：Tauri 桌面壳、前端账本会话、本地任务账本、发布更新与联合冒烟；一级子代办与 schema v5 自 `v0.1.2` 起已进入公开版本，`v0.1.5` 已于 2026-07-27 正式发布，其折叠和子列表视觉调整不改变架构边界

## 1. 目标

当前模块边界不引入新的任务状态；界面层已经把“现在做”和“接下来”合并为统一待办列表，并在顶层待办下支持一级子代办。顶层与子项共用可靠写链路，界面只负责展开、编辑和提交意图。目标是让后续增加“稍后再做、被卡住、不再做和完整周报”时，不需要继续把逻辑堆进单个入口文件：
...
5. 顶层/子项创建、标题修改、完成、删除、撤销、各自同级重排和跨版本迁移都由同一条可靠写入链路处理，界面不维护第二份任务事实。
6. 更新检查与安装通过项目自有命令和 Rust 适配层收口，smoke 在前后端两层禁止访问更新网络。
7. “复制本周完成”复用现有只读账本查询，并通过项目自有剪贴板端口隔离 Tauri 插件；它不进入可靠写操作箱，也不扩展任务状态机。
...
- 通用工程能力：Tauri IPC、窗口控制、SQLite、浏览器存储、系统剪贴板、页面渲染、测试运行器。这些能力通过边界适配，不进入领域模型。
...
│  │  ├─ subtask-controller.js  子代办交互控制器：展开、单次添加、标题编辑、同组重排与焦点恢复
│  │  ├─ list-search-controller.js 当前列表搜索、输入法与搜索焦点临时状态
│  │  ├─ weekly-completion-controller.js 本地自然周计算、完成事实校验与 Markdown 编排
...
│  │     ├─ clipboard-writer.js 系统剪贴板文本写入适配器
...
│  └─ ledger/
│     ├─ domain.rs             实体、一级层级不变式、错误码及创建/修改/完成/删除/重排的纯领域转换
│     ├─ service.rs            TaskService 应用服务、顶层/子项用例编排与 LedgerStore 端口
│     ├─ commands.rs           账本 Tauri 薄命令与错误映射
│     ├─ mod.rs                LedgerState 门面及桌面运行时装配类型
│     ├─ sqlite.rs             SqliteLedgerStore、父子投影/事实查询、原子提交与迁移前一致性备份
│     ├─ sqlite/schema.rs      数据库身份、schema v5 与 v1 → v2 → v3 → v4 → v5 结构迁移
│     ├─ sqlite/integrity.rs   队列/层级/事件/奖励与数据库投影、元数据、回执一致性验证
...
flowchart TD
    ...
    UI --> SUBTASK["SubtaskController 子项展开 / 添加 / 编辑 / 同组重排"]
    FE --> LIST
    FE --> SUBTASK
    LIST -->|"onUpdateTitle / onReorder 意图"| SESSION
    SUBTASK -->|"onCreate / onUpdateTitle / onReorder 意图"| SESSION
...
- `SubtaskController` 只维护父组展开、单次添加、子项标题编辑、组内重排预览、删除确认与焦点恢复；新增和重排通过注入回调进入 `LedgerSession`，完成、删除和撤销继续由组合根把稳定任务/事件 ID 交给同一会话。它不复制父项级联、奖励或层级规则。
...
- `WeeklyCompletionController` 只在用户点击后计算本地自然周、调用 `TauriGateway.weeklyFacts()`、校验有效完成事实并生成 Markdown；它不提交账本写命令，也不把完整周报状态放进页面。
...
- `TauriClipboardWriter` 是 `tauri-plugin-clipboard-manager` 的前端适配器，只暴露 `writeText`；主窗口 capability 只授予 `clipboard-manager:allow-write-text`，不授予读取、清空或图片权限，能力缺失时显式失败且不静默回退到浏览器剪贴板。
...
| 边界 | 关键输入 | 关键输出 | 责任 |
|---|---|---|---|
| 页面 → `LedgerSession` | 新顶层/子项标题、父项 ID、待修改标题/期限与任务 ID、要完成或删除的任务 ID、顶层或同组子项重排前后顺序、完成事件 ID | 操作结果或显式错误 | 接收用户意图，不自行改变领域事实 |
...
| DOM → `SubtaskController` | 父组展开、单次添加、子项标题编辑、组内拖动/键盘移动、删除组确认 | `parentTaskId + title`、`taskId + title` 或父项范围内的完整重排意图 | 只维护子项临时交互；完成/删除/撤销仍复用组合根的统一任务动作 |
| DOM → `ListSearchController` | 当前页面、`Ctrl+F` / 菜单、搜索输入、输入法组合事件、取消 / `Esc` | `{ panel, query }` 临时视图状态 | 只筛选当前页面标题和管理焦点；不提交账本命令 |
| DOM → `WeeklyCompletionController` | 用户点击“复制本周”、当前本地时间 | 本地自然周半开区间、复制结果或显式错误 | 只编排只读查询、Markdown 和提示；不进入账本写入链路 |
| `WeeklyCompletionController` → 网关 / 剪贴板端口 | `fromMs`、`toMs`、已校验的 Markdown 文本 | `weekly_facts` 或文本写入结果 | 复用现有 IPC 查询并隔离 Tauri 剪贴板插件 |
...
| Tauri 命令 → `TaskService` | Rust 字符串/ID、父项 ID、标题/期限修改与删除目标、顶层或子项调整前后 ID 列表 | 领域对象或稳定 `CommandError` | 薄适配，不复制业务判断 |
...
当前源码定义的 IPC 名称与 payload 是跨语言协议，重构时保持稳定：`runtime_profile`、`report_frontend_ready`、`window_status`、`set_window_mode`、`hide_to_tray`、`capture_task`、`create_subtask`、`update_task_title`、`update_task_deadline`、`complete_task`、`delete_task`、`reorder_tasks`、`reorder_subtasks`、`undo_completion`、`ledger_snapshot`、`weekly_facts`、`ledger_integrity`、`check_for_update`、`install_update`。子项新增协议为 `create_subtask({ parentTaskId, title, operationId })`；标题修改、完成、删除和撤销按稳定任务/事件 ID 复用原命令；子项重排协议为 `reorder_subtasks({ parentTaskId, movedTaskId, expectedTaskIds, orderedTaskIds, operationId })`。期限修改只允许顶层立即待办，`deadlineOn` 只能是 `null` 或 `YYYY-MM-DD`。窗口状态另通过 `window-status-changed` 事件由 Rust 主动同步。新增能力应扩展网关与薄命令，不能让页面散落调用字面量。
...
| 父组展开、子项单次添加/编辑/同组排序与焦点恢复 | `SubtaskController` | 只维护临时界面状态；父子关系、级联完成、顺序和奖励由 Rust 账本掌握 |
...
`LedgerSnapshot` 的父子读取契约也有明确分工：

- `queue` 与 `currentTask` 只包含立即可执行的顶层待办，主列表只从这里建立父组；子项永远不会独立进入顶层队列。
- `subtasks` 返回所有 `parentTaskId` 非空且状态为 `pending` / `completed` 的未软删子项，不要求父项仍在队列。父项完成或软删后，子项投影继续保留，以便完成历史得到真实组总数；主列表仍因父项不在 `queue` 而隐藏该组。
- `effectiveCompletions` 通过任务投影的 `activeCompletionEventId` 连接事件表，返回全部当前有效的顶层与子项完成事实，按完成时间和事件序号倒序。它不受审计窗口影响。
- `events` 仍是最近 100 条审计事件，只用于有限历史检查；完成记录、父子组标题和总数不得从这段截断日志反推。
...
2. 正常模式打开应用数据目录中的 SQLite 和正常窗口状态文件；v5 文件验证身份与结构，v1、v2、v3、v4 磁盘文件先备份再按级迁移到 v5；冒烟模式使用内存账本与独立窗口状态文件。
...
假设顶层待办 A 仍在队列中，用户点击父行固定 `＋`，依次添加子项 A1、A2：

1. `SubtaskController` 只维护 A 的展开、单次输入和焦点；有效标题按 `Enter` 或失焦通过 `onCreate(parentTaskId, title)` 交给 `LedgerSession.createSubtask`，创建成功后退出输入态、保持展开并恢复父行固定 `＋`，不会先把临时子项写入前端快照。零子项父项取消空白新增时收起空区域，已有子项时只退出输入态。
2. 会话生成稳定 `operationId` 并保存操作箱；`TauriGateway` 调用 `create_subtask({ parentTaskId, title, operationId })`，薄命令经 `LedgerState` 进入 `TaskService.create_subtask`。
3. 应用服务读取父项 A；领域转换要求 A 是可执行顶层待办，生成带 `parentTaskId=A`、正数 `siblingPosition`、无期限/队列位置/延期字段的子项，以及 `subtask_created` 事件。SQLite 在提交时把新子项追加到该父项所有既有位置之后。
4. 子项标题修改复用 `update_task_title`；完成与软删除复用 `complete_task`、`delete_task`；完成记录中的撤销复用 `undo_completion`。子项事件 metadata 保存 `parentTaskId` 和事件发生时的 `parentTitle`，前端不猜父级关系。
5. 组内拖动或 `Alt + ↑/↓` 由控制器提交父项 ID、移动子项、完整旧顺序和完整新顺序；`reorder_subtasks` 只排列同一父项当前未软删的子项，并保留软删子项占用的历史位置。
6. 用户勾选父项时，前端只提交一次 `completeTask(parentTaskId)`。`TaskService` 读取全部活动子项，领域层为每个 `pending` 子项生成伴随完成写入；SQLite 在 `BEGIN IMMEDIATE` 内用 `SubtasksUnchanged` 精确比较全部活动子项的 ID、版本、位置和状态（包括空集合），再依次写子完成事件、父完成主事件、父奖励和唯一外部命令回执。已完成子项不重复写，已软删除子项忽略，任一步失败都整组回滚。单独完成最后一个子项仍不会自动完成父项。

输出：主列表仍只按 `snapshot.queue` 渲染顶层父项，子项来自 `snapshot.subtasks` 并按父项 ID、同级位置归组；子项的增删改、完成、撤销和重排全部进入统一账本命令链。
...
4. 应用服务检查幂等回执并读取 B；`update_task_title_transition` 接受立即顶层待办，或活动父项下的未完成子项，通过 `TaskWrite::Update` 更新标题与版本并生成 `title_updated`。metadata 保存 `beforeTitle` / `afterTitle`，子项事件还保存父项元数据。
...
假设统一列表仍有 A、B，用户点击 A 或其子项行右侧的删除入口：
...
3. 应用服务读取目标并检查幂等回执；`delete_task_transition` 将投影改为 `abandoned`，固定记录 `reason=用户删除` 与 `metadata.action=delete`，通过 `TaskWrite::Update` 生成变化集。删除子项时保留它的 `siblingPosition`；删除含子项的父项前由界面确认，但领域层只软删父项，不级联改写子项。
4. `SqliteLedgerStore::commit_transition` 在同一事务中更新任务投影、追加 `abandoned` 事件并保存命令回执；删除不产生奖励交易，也不物理删除任务或旧事件。
5. 命令确认并刷新真实快照后，目标从主列表或父组中消失。父项软删时，它不再出现在 `snapshot.queue`，但未软删子项仍保留在 `snapshot.subtasks`，供完成历史恢复真实组总数。

输出：用户界面完成删除，账本事实仍可审计；删除沿用 `abandoned` 类型，不产生奖励，也不物理删除父项、子项或事件。
...
### 第 5 轮：v1、v2、v3 或 v4 账本首次由当前源码打开

1. `SqliteLedgerStore` 在迁移事务开始前识别旧 `user_version`；磁盘文件用 `VACUUM INTO` 生成一致性备份，并回读 `quick_check` 与备份版本，备份不合格时停止打开原账本。
2. 迁移器按版本逐级执行：v2 增加队列重排，v3 增加标题修改，v4 增加可空期限，v5 再增加 `parent_task_id`、`sibling_position` 和子代办事件类型；不会猜测或跳过未知路径。
3. v4 → v5 显式重建任务与事件表：既有任务全部作为顶层任务迁入，父项与同级位置为 `null`；事件序号、命令回执、奖励、索引、外键和迁移记录继续保留。
4. 需要重建表时在事务外切换外键设置，再以 `BEGIN IMMEDIATE` 执行迁移；每级迁移记录版本，最终写入 `user_version=5`，随后恢复外键并执行结构和完整性检查。
5. 已是 v5 的账本只做身份和结构验证；高于当前支持版本或不在迁移路径中的文件显式拒绝，不会按旧结构静默打开。

输出：旧数据升级前有可验证的一致性快照，v1 至 v4 历史按级进入 v5，旧事件与新增子代办事件类型共存，未知未来版本失败快。
...
`integration_smoke.rs` 等待前端就绪后，再通过生产状态和服务验证窗口模式、隐藏后从托盘恢复、账本幂等、标题/期限修改、顶层与子项创建/完成/重排/撤销/删除，以及完整性。它只能消费生产接口，不能成为运行配置或前端探针的依赖。
...
9. 一次领域变化只能进入一个 `commit_transition`；一个父项级联可以包含多份受检任务写入和伴随事件，但任务、队列位置、全部事件、奖励和唯一外部主回执仍必须同事务提交或回滚。
...
15. `completeTask(taskId)` 可以完成队列中的任意顶层待办或活动父项下的任意未完成子项；“第一项”只决定当前展示。目标是父项时，Rust 必须在同一事务中完成全部未完成有效子项再完成父项；最后一个子项单独完成仍不得自动完成父项。
16. 顶层重排和同组子项重排都必须提交被移动任务、完整旧顺序和完整新顺序；领域层、应用服务与存储层逐级校验同一任务集合、父项范围、实际位移和并发版本，不能接受局部或猜测顺序。
17. `queue_reordered` 与 `subtasks_reordered` 分别是顶层和子项顺序变化的账本事实；软删子项保留原 `sibling_position`，活动子项只在既有活动位置集合内重排。
18. `TaskListController` 与 `SubtaskController` 可以临时替换 DOM、预览重排和恢复焦点，但只能通过回调把意图交给 `LedgerSession`，不得持有 Gateway 或其他写入依赖。
19. schema v1 至 v4 磁盘文件只能经迁移前 `VACUUM INTO` before-v5 备份和受检的逐级路径升级到 v5；schema v5 需验证身份、层级字段和事件约束，未来高版本必须显式拒绝。
20. `updateTaskTitle(taskId, title)` 允许修改立即顶层待办或活动父项下的未完成子项；必须以 `TaskWrite::Update + title_updated` 保存修改前后标题，子项还保存父项元数据，不改变状态、顺序或金币，也不得直接覆盖旧事件。
...
22. `deleteTask(taskId)` 是历史保留型删除：顶层或子项都写成 `TaskWrite::Update + abandoned`，固定保留“用户删除”原因且不产生奖励。删除父项不级联删除子项；删除子项不回收历史位置；禁止物理删除任务、历史或奖励行。
23. 层级只能有一级：顶层任务没有 `parentTaskId` / `siblingPosition`；子项必须指向顶层父项并持有正数 `siblingPosition`，且不得携带顶层队列、期限、延期或阻塞字段。禁止子项再成为父项。
24. 顶层完成产生 `+1`，撤销顶层完成产生 `-1`；子项完成和撤销都不产生奖励。父项完成时禁止直接撤销其子项，必须先撤销父项。
25. `snapshot.subtasks` 与 `snapshot.effectiveCompletions` 是父子组当前事实；有限的 `snapshot.events` 不能作为它们的回退来源。周回顾同样直接读取当前有效完成投影，不扫描截断日志猜结果。
26. 托盘恢复入口必须统一进入 `Expanded`；窗口模式只能由 Rust 适配层改变，并以 `window-status-changed` 把真实状态同步到 DOM。
27. 自动更新只允许 normal 模式访问网络；前端必须不启动更新调度并隐藏入口，Rust 还必须在构造 updater 前独立校验运行模式。任何一层都不得以旧网络路径作为静默回退。
28. 安装只能针对 `AppUpdateService` 当前保存的精确待安装版本，并由单次安装锁串行化；页面不得绕过 `TauriGateway` 直接调用 updater 插件。
29. 更新产物签名由 Tauri updater 校验；发布私钥只存在于受控的发布环境和备份中，绝不能进入仓库。Windows Authenticode 与 updater 签名是两条独立信任链，不能互相替代。
...
| 子代办交互控制器 | `SubtaskController` | 隔离父组展开、单次添加、子项编辑/同组重排和焦点恢复，不复制层级与奖励规则 |
...
- 主界面采用统一待办列表：顶层立即待办可在原行修改标题与可选期限，每项都可直接勾选完成，第一项只表达当前优先项。父项下可展开一级子代办，单次添加、修改、逐项完成/撤销、软删除和同组拖动/键盘重排由 `SubtaskController` 编排；父项完成由 Rust 原子级联全部未完成子项，完成记录按父组折叠显示真实进度。待办与完成记录搜索由独立 `ListSearchController` 和纯选择器承担，不进入账本。
- 子项写链路已经贯通：`create_subtask` 与 `reorder_subtasks` 有独立 payload 和命令；`update_task_title`、`complete_task`、`delete_task`、`undo_completion` 复用稳定任务/事件 ID。所有命令都经过 `SubtaskController / app.js → LedgerSession → 操作箱 → TauriGateway → Tauri 薄命令 → TaskService → 领域转换 → SqliteLedgerStore::commit_transition`，界面没有第二份父子状态机。
...
- SQLite 当前源码 schema 为 v5；任务投影增加 `parent_task_id` 和 `sibling_position`，事件约束增加子项创建/完成/撤销/重排类型。打开 v1 至 v4 磁盘文件时先以 `VACUUM INTO` 创建并验证 before-v5 一致性备份，再按受检路径逐级迁移到 v5，未来高版本显式拒绝。
- 前端使用原生 ES 模块、JSDoc 和 Node 内置测试，不增加运行时依赖。
- `LedgerSnapshot.subtasks` 返回所有未软删子项，即使父项已完成或软删；主列表仍只按 `queue` 中的顶层父项投影。`effectiveCompletions` 通过 `activeCompletionEventId` 返回不受 100 条审计窗口影响的全部当前有效完成事实；完成历史不再从截断 `events` 猜测父子组。
- `WeeklyCompletionController` 复用 `weekly_facts` 查询 `[本周一 00:00, 下周一 00:00)` 的当前有效完成事实：顶层完成必须发生在本周，普通父组只纳入本周子完成；父项本周完成时补齐该父项所有仍有效的跨周子完成。它生成固定 Markdown，空结果不会写剪贴板。
- Rust 入口拆为组合根、桌面模块、运行配置、前端探针、联合冒烟和账本分层。
- SQLite 结构迁移、完整性检查、运行时适配与测试分别落到独立模块。
- 原有 IPC 协议、操作箱 v1 key 和数据库事务语义保持兼容，并以 `create_subtask`、`reorder_subtasks` 扩展一级子代办能力；用户可见界面继续按任务优先原则精简。
...
- Windows x64 NSIS 配置、发布版本一致性检查和 GitHub Actions 发布工作流已经落地；`v0.1.5` 已于 2026-07-27 正式发布并成为公开 Latest。当前正式安装基线仍为 `v0.1.4`，版本间应用内更新闭环仍待用户实机验证。
- 一级子代办及其 schema v5、完成历史和周回顾语义自 `v0.1.2` 起已经进入公开版本；`v0.1.5` 只统一两页折叠入口、透明子列表、组内短轨道和行级反馈，不新增领域状态、IPC 命令或持久化格式。
...
2. 在已实现的“复制本周完成”最小切片上增加完整历史和非 AI 周报；继续补今天/上周/自定义区间、进行中/阻塞/调整分组、选择纳入项和纯文本输出。若后续需要“指定当前”，应复用现有完整队列重排语义，而不是增加第二套当前任务状态。
3. 在现有 v1 至 v4 迁移前备份之外，增加用户可管理的定期备份、导出和恢复校验，继续保持失败快和无静默旧链路。
