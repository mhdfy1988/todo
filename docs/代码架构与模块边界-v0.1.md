...
> 更新时间：2026-07-22
> 适用范围：Tauri 桌面壳、前端账本会话、本地任务账本、发布更新与联合冒烟；不代表阶段 1 全部产品能力或首次线上发布已经完成
...
输出：normal 模式可以由用户控制安装新版本；smoke 通过前端不开启、Rust 失败快的双层门禁保持无更新网络访问。当前本地安装器与 `.sig` 已生成，线上 `latest.json` 和完整升级闭环仍需在首个 GitHub Release 后验证。
...
- Windows x64 NSIS 配置、发布版本一致性检查和 GitHub Actions 发布工作流已经落地；本机已成功生成 `setup.exe` 与 `.sig`，统一门禁通过，具体测试数量留到最终发布复核。

### 发布前待完成

- 创建首个 GitHub Release，确认安装器、`.sig` 与 `latest.json` 均按预期上传，并完成一次真实首次安装。
- 通过后续版本完成一次线上“发现更新 → 用户确认 → 下载校验 → 安装重启”闭环验证；当前不能把本地打包成功写成线上更新已通过。
