...
- [ ] RELEASE-0.1.5 收口版本与文档、推送 main 并完成正式 GitHub Release
- [ ] UPDATE-0.1.5 从重新读取到的真实正式安装基线完成应用内发现、下载校验、安装重启与数据保留实测

## 当前指针
- 阶段状态：`v0.1.5` 视觉候选与隔离测试包已通过用户手感确认；当前公开 Latest 与 Windows 正式安装基线均为 `v0.1.4`
- 当前正在做：同步标准 CHANGELOG、版本事实和项目文档，运行正式发布门禁后触发 GitHub Actions 签名发布
- 完成后下一项：核对 `v0.1.5` Release、标签、安装器、`.sig`、`latest.json`、匿名公开下载和更新正文，再从真实安装基线安排应用内更新实测
- 边界：公开发布产物不可覆盖；`v0.1.5` 一旦正式发布，后续修复必须提升版本，不能替换既有资产
