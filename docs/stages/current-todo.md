...
- [x] PACKAGE-0.1.4 完成隔离功能候选试装并确认交互手感；试装包元数据仍为 `0.1.3`，随后正式发布提交已统一提升到 `0.1.4`
- [x] UPDATE-0.1.3-REPLAN 明确旧 `0.1.3` 更新实测未形成闭环，并由最新 `0.1.4` 目标取代
- [x] RELEASE-0.1.4 收口版本文档、推送 main 并完成正式 GitHub Release
- [x] UI-FEEDBACK-15 统一两页父子折叠入口，并收口透明子列表、短轨道、行级悬浮和已完成子项留白
- [x] PACKAGE-0.1.5 重建隔离测试包并完成用户手感确认
- [x] RELEASE-0.1.5 收口版本与文档、推送 main 并完成正式 GitHub Release
- [ ] UPDATE-0.1.5 从重新读取到的真实正式安装基线完成应用内发现、下载校验、安装重启与数据保留实测

## 当前指针
- 阶段状态：`v0.1.5` 已通过签名发布闭环成为 GitHub Latest；当前 Windows 正式安装基线仍为 `v0.1.4`
- 当前正在做：从已安装的 `v0.1.4` 发起 `v0.1.5` 应用内更新，核对发现、下载校验、安装重启和数据保留
- 完成后下一项：在升级后的正式窗口核对版本、“待办”名称、悬浮任务牌图标、父子折叠与列表视觉、原有父子数据、完成历史、托盘单实例和再次检查更新结果
- 边界：`v0.1.5` 公开发布产物不可覆盖；后续修复必须提升版本，不能替换既有资产
...
- RELEASE-0.1.4 正式结果：[Actions #30064634767](https://github.com/mhdfy1988/todo/actions/runs/30064634767) 已完整成功；[`v0.1.4`](https://github.com/mhdfy1988/todo/releases/tag/v0.1.4) 已成为 GitHub Latest，Release、标签和工作流均指向 `d9755121ad3ab7b0952ed9940361abe4a248b443`。公开资产共 3 项，安装器大小为 `3,200,349` 字节，SHA-256 为 `D9B4550F2DF67C0045C2454D599E0A04ACA5D364452D0134B1D8203C2DCA9CC4`；工作流验证、匿名公开下载、`.sig`、内嵌公钥验签、`latest.json` 和 CHANGELOG 正文核对均通过
- UI-FEEDBACK-15 完成：待办页和已完成页都通过父项右侧进度信息与箭头展开或收起，页面切换保留用户选择；子列表常态透明，用缩进和短轨道表达层级，子项悬浮或键盘聚焦时显示 120ms 轻量背景，已完成子项高亮区域补足前后留白
- PACKAGE-0.1.5 候选：隔离安装身份“待办测试版”完成多轮真实手感验证；最终候选只改变前端视觉与契约测试，不改 schema、IPC、领域状态、正式 identifier、更新端点或签名公钥
- RELEASE-0.1.5 本地门禁：版本与标准 CHANGELOG 检查、前端 158/158、Rust 68/68、Rustfmt、Clippy、桌面联合 smoke 和 release 构建均通过；正式配置 EXE 的 ProductName/FileDescription 为“待办”，ProductVersion/FileVersion 为 `0.1.5`
- RELEASE-0.1.5 正式结果：[Actions #30211980913](https://github.com/mhdfy1988/todo/actions/runs/30211980913) 已完整成功；[`v0.1.5`](https://github.com/mhdfy1988/todo/releases/tag/v0.1.5) 已成为 GitHub Latest，Release、轻量标签和工作流均指向 `533e3450e03455871202f31a70b4b74fc9bf1acb`。公开资产共 3 项，安装器大小为 `3,199,710` 字节，SHA-256 为 `FF6632FD35A65F383DCB9E02E5FB24F608644100A236C844975C493D79991574`；工作流验证、匿名公开下载、两种 Windows 元数据映射、`.sig`、客户端内嵌公钥验签、Release 正文与 CHANGELOG notes 核对均通过
- UPDATE-0.1.4 当前基线：发布后重新读取 Windows 卸载注册信息，正式安装仍登记为“代办”`0.1.3`、位置 `D:\代办`；该记录只证明当前版本，不冒充此前升级方式，下一步从这一真实基线验证到 `v0.1.4`
- UPDATE-0.1.5 重排：`v0.1.4` 的应用内更新实测未在 Latest 切换前形成闭环；正式发布 `v0.1.5` 后不再伪造旧目标，改为重新读取当时真实安装基线并直接验证到 `v0.1.5`
- UPDATE-0.1.5 当前基线：2026-07-27 重新读取 Windows 卸载信息，正式“待办”为 `0.1.4`、安装位置 `D:\待办`；测试版 `0.1.4` 使用独立安装身份，不作为正式更新起点
