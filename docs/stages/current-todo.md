...
- [ ] UPDATE-0.1.2 从已安装的 `v0.1.1` 完成应用内升级、schema v4→v5 与数据保留实测

## 当前指针
- 阶段状态：`v0.1.2` 已于 2026-07-24 通过签名发布闭环正式发布为 GitHub Latest；当前用户安装基线仍为 `v0.1.1`
- 当前正在做：从已安装的 `v0.1.1` 发起 `v0.1.2` 应用内更新，核对更新发现、下载校验、安装重启、schema v4→v5 迁移前备份和原有数据
- 完成后下一项：在升级后的正式窗口核对一级子代办、“复制本周”、托盘单实例、再次检查更新结果和日常使用手感
- 边界：公开发布产物不可覆盖；若升级实测发现问题，必须提升版本修复，不能替换 `v0.1.2` 资产
